  ...
  <div class="breadcrumb-wrapper" style="background-image: url({{ asset('assets/images/blog/blog-thumb.png') }})">
    <div class="container">
      <div class="breadcrumb-content">
        <h1 class="breadcrumb-title">{{ __('blog.title') }}</h1>
          ...
          <div class="breadcrumb-menu-wrap">
            <div class="breadcrumb-menu">
              <ul>
                ...
                <li aria-current="page">{{ __('blog.title') }}</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div class="row">
        <div class="col-lg-8">
          <div class="techin-blog-header">
            <a class="techin-showing-result" href="#">{{ __('blog.showing_result', ['first' => $blogs->firstItem() ?? 0, 'last' => $blogs->lastItem() ?? 0, 'total' => $blogs->total() ?? 0]) }}</a>
            <div class="teching-slect-wrapper">
              <select>
                <option data-display="{{ __('blog.sort_by_latest') }}">{{ __('blog.sort_by_latest') }}</option>
                <!-- Add sorting options if needed, for now keeping placeholder or removing hardcoded options if they don't do anything -->
              </select>
            </div>
          </div>
          <div class="row">
            @forelse($blogs as $blog)
              @php
                ...
                $title = $translation->title ?? optional($fallbackTranslation)->title ?? 'Blog Post Title';
                ...
                // Handle image path like in about-us page
                ...
                if ($imagePath && (str_starts_with($imagePath, 'http://') || str_starts_with($imagePath, 'https://'))) {
                    ...
                    $imageUrl = asset('assets/images/blog/img' . (($loop->index % 8) + 1) . '.png');
                }
              ...
              <div class="col-lg-6">
                <div class="blog-wrapper">
                  ...
                  <div class="blog-img">
                    ...
                    <a class='techin-default-btn blog-btn1' data-text='{{ __('blog.read_more') }}' href='{{ $permalink }}'>
                      <span class="button-wraper">{{ __('blog.read_more') }}</span>
                    </a>
                  </div>
                </div>
              </div>
          ...
          <aside class="sidebar-area pl-25">
            <div class="widget widget_search">
              ...
              <form class="search-form">
                <input type="search" placeholder="{{ __('blog.search_placeholder') }}">
            ...
            <div class="widget">
              ...
              <div class="recent-post-wrap">
                @forelse($recentPosts as $recentPost)
                  @php
                    $recentTranslation = optional($recentPost->translation);
                    $recentFallbackTranslation = $recentPost->translations->firstWhere('locale', config('app.fallback_locale'));
                    $recentTitle = $recentTranslation->title ?? optional($recentFallbackTranslation)->title ?? 'Recent Blog Post';
                    ...
                    $recentPermalink = \Illuminate\Support\Facades\Route::has('blog.show')
                  ...
                  <div class="recent-post">
                    <div class="media-img">
                      <a href='{{ $recentPermalink }}'>
                        <img src="{{ $recentImageUrl }}" alt="{{ $recentTitle }}">
                      </a>
                    </div>
                    <div class="media-body">
                      <div class="recent-post-meta">
                        ...
                        <div class="post-title">
                          <a class='text-inherit' href='{{ $recentPermalink }}'>{{ Str::limit($recentTitle, 50) }}</a>
                        </div>
                      </div>
                    </div>
                  </div>
