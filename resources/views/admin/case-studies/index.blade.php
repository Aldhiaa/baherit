    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('admin.Case Studies') }}
            </h2>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                {{ __('Manage your case studies and success stories') }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Total Case Studies') }}</h3>
                        <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $caseStudies->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $caseStudies->where('is_featured', true)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Industries') }}</h3>
                        <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $caseStudies->pluck('industry')->unique()->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="relative w-full md:w-64">
                        ...
                        <input type="text" id="searchInput" placeholder="{{ __('Search case studies...') }}"
                    ...
                    <div class="flex space-x-2">
                        <select class="block w-full md:w-40 py-2 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            @foreach($caseStudies->pluck('industry')->unique() as $industry)
                            ...
                            @forelse($caseStudies as $caseStudy)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150 ease-in-out">
                                        ...
                                        <div class="flex items-center">
                                            @if($caseStudy->image_url)
                                                <div class="flex-shrink-0 h-10 w-10 bg-blue-100 dark:bg-blue-900/50 rounded-lg flex items-center justify-center">
                                                    <img src="{{ $caseStudy->image_url }}" alt="{{ $caseStudy->getTitleInLocale('en') }}" class="h-10 w-10 object-cover rounded-lg">
                                                </div>
                                            @else
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900 dark:text-white">
                                                    {{ $caseStudy->getTitleInLocale(app()->getLocale()) }}
                                                </div>
                                                <div class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs">
                                                    {{ Str::limit($caseStudy->getDescriptionInLocale(app()->getLocale()), 50) }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {{ $caseStudy->completed_at ? $caseStudy->completed_at->format('M d, Y') : __('N/A') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <a href="{{ route('admin.case-studies.show', $caseStudy) }}"
                                            ...
                                            <form action="{{ route('admin.case-studies.destroy', $caseStudy) }}"
                                                  method="POST"
                                                  class="inline-block"
                                                  onsubmit="return confirm('{{ __('Are you sure you want to delete this case study?') }}');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-1">{{ __('No case studies found') }}</h3>
                                            <p class="text-gray-500 dark:text-gray-400 mb-4">{{ __('Get started by creating a new case study.') }}</p>
                                            <a href="{{ route('admin.case-studies.create') }}"
