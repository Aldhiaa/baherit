    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                {{ __('Manage your services and offerings') }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $services->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $services->pluck('category')->unique()->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('With Icons') }}</h3>
                        <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $services->whereNotNull('icon')->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="relative w-full md:w-64">
                        ...
                        <input type="text" id="searchInput" placeholder="{{ __('Search services...') }}"
                            ...
                            @forelse($services as $service)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150 ease-in-out">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            @if($service->icon)
                                                <div class="flex-shrink-0 h-10 w-10 bg-blue-100 dark:bg-blue-900/50 rounded-lg flex items-center justify-center">
                                                    <i class="{{ $service->icon }} text-blue-600 dark:text-blue-400 text-lg"></i>
                                                </div>
                                            @else
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900 dark:text-white">
                                                    {{ $service->title[app()->getLocale()] ?? $service->title['en'] ?? $service->title }}
                                                </div>
                                                <div class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs">
                                                    {{ Str::limit($service->description[app()->getLocale()] ?? $service->description['en'] ?? $service->description, 50) }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white">
                                            @if($service->features)
                                                {{ count($service->features) }} {{ __('features') }}
                                            @else
                                                {{ __('No features') }}
                                            @endif
                                        </div>
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-1">{{ __('No services found') }}</h3>
                                            <p class="text-gray-500 dark:text-gray-400 mb-4">{{ __('Get started by creating a new service.') }}</p>
                                            <a href="{{ route('admin.services.create') }}"
