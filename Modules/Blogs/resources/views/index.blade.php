			...
			<div class="row">
				<div class="col-lg-6">
					<div class="breadcrumb-content">
						<div>
							<h2>
								<img src="{{ asset('assets/svg/title-effect2.svg') }}" class="img-fluid" alt="title-effect" />{{ __('messages.Blogs') }}
							</h2>
							<p>
						...
						<div class="feature-box" data-aos="fade-up" data-aos-delay="{{ $loop->index * 50 }}">
							<div class="feature-top">
								<div class="feature-icon">
									@if (!empty($post->image))
										<img src="{{ asset('storage/' . $post->image) }}" class="img-fluid outline-icon" alt="{{ $post->title }}">
									@else
										<img src="{{ asset('assets/svg/blog-vector.svg') }}" class="img-fluid outline-icon" alt="blog" />
										<img src="{{ asset('assets/svg/blog-post-amico.svg') }}" class="img-fluid outline-icon" alt="blog" />
									@endif
								</div>
								<h3>{{ $post->title }}</h3>
							</div>

							<h4 class="clamp-3">{!! $post->short_description ?? \Illuminate\Support\Str::limit(strip_tags($post->body ?? ''), 120) !!}</h4>
